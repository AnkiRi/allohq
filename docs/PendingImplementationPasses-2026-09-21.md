...
| --- | --- | --- | --- | --- |
| 2026-09-23T05:19:18Z | _(branch `studio-defects-and-harness`, draft PR)_ | **implemented, pending review** | **Product staleness had two root causes, both fixed.** (1) `bindProduct` spread the old props and overwrote only `productId`, so the block said product B while still carrying A's title, price, image and variant — the inspector and outline read those directly, and `resolveProduct` falls back to them whenever the store lookup misses. Saving re-ran enrichment and 'fixed' it, which is what made a bug look like eventual consistency. (2) Preview responses were applied unconditionally, so a slower earlier render could land after a faster later one and put the previous state back. Renders are now sequenced and stale ones dropped. **Asset storage made honest:** production refused uploads and generation at the END of the work with an error naming `ASSET_BUCKET` and `ASSET_CDN_BASE_URL` — incomplete, because the S3 client also needs credentials. A capability check now reports every missing piece to operators, gates generation BEFORE a provider is chosen, and shows merchants a sentence naming no variable, bucket or provider. **Ask Joon understands visual intent:** 'put this snowboard in the hands of a model' went down the copy path and surfaced a storage error; it now returns a structured proposal — product, mode, target block, provider, estimated cost — and generates nothing until the merchant accepts. A product block is never the target, because its image is resolved from Shopify at send. **Model harness extended, not replaced:** a capability-aware registry with 8 text and 4 visual workloads, per-model API ids, input/output modes, cost class, and availability requiring credential AND explicit enable flag. Fallback may only move within the requested capability, so reference-grounded work can never silently become an invented image. Claude is absent from every image capability because the API returns no images, asserted by test. Nano Banana 2 `gemini-3.1-flash-image` and Pro `gemini-3-pro-image` registered. typecheck 19/19, unit 602/602, integration 91/91, build green. | **Draft PR only — not merged, not deployed.** **Image generation must not be called operational:** provider output → persisted object → Studio asset → block → save/reload → preview → approved version has never completed, because durable storage is unconfigured in production. The registry is a routing and capability layer; Gemini and Anthropic adapters are declared but their network calls are not implemented in this pass, and no provider secret was set. Image analysis/OCR is registered as a capability with no adapter that reads text from an image, and nothing claims otherwise. Configuration at `docs/ModelHarnessAndStorageConfiguration-2026-09-23.md` |
| 2026-09-22T14:55:10Z | _(branch `email-studio-block-ai`, draft PR #36)_ | **Studio scope complete for manual review** | **Collections resolve for real.** `resolveBlockData` in campaign-engine returns products plus each bound collection's contents, ordered by the join table's position, limited per block and scoped to the store; Studio preview, the approval snapshot and the delivery worker all call it, so a merchant cannot approve one set of products and have another sent. Collection binding is LIVE by design and is labelled as such with what the collection holds today. The approval snapshot records what each collection resolved to, keeping the send auditable without freezing the binding. Variant selection added where a product has more than one. **Reference-grounded image generation built.** `VisualProvider` declares whether a provider is configured and whether it accepts the real product image as an INPUT; two reference-capable adapters added (Flux Kontext, OpenAI gpt-image-1), each an explicit opt-in on top of its API key. With nothing configured, generation fails closed naming the variables and substitutes no stock imagery. Spend now has two ceilings, workspace-daily and per-email-lifetime. **EXIF/metadata stripping implemented** on both upload and generated-image paths, proven on bytes. **Reordering made keyboard-accessible** — it was `group-hover` only, so a keyboard user could not move a block at all. Preflight gained contrast (measured only where a block states both colours) and a generated-imagery note stating the OCR limitation. **Verified in a real browser** via CDP against the actual components: the formerly broken email opens, Alt+Arrow genuinely reorders, a collection binds and is labelled live, block scope is the default and whole-email requires an explicit switch, generation fails closed, preflight renders, and 390px has no horizontal overflow. Nine screenshots captured. typecheck 19/19, unit 554/554, integration 90/90, build green. | **Draft PR only — not merged, not deployed.** **The reference-generation network call has not been made against a live provider**: no credentials here and paid generation is not run in CI, so selection, capability reporting, prompt construction and failure handling are tested but the first real call is a manual acceptance step. The browser pass drove the real components with fixtures at the network boundary, because a Clerk session is a credential this environment lacks and production Clerk must not be used for testing; auth and the database round trip are covered by the integration suite instead. Still absent and claimed nowhere: malware scanning, content moderation, OCR, and real email-client rendering evidence. Configuration reference at `docs/EmailStudioConfiguration-2026-09-22.md` |
| 2026-09-22T13:51:44Z | _(PR #37 `fix-email-delivery-safety`; PR #36 stays draft)_ | **split out for separate review** | The four delivery/editor safety fixes were separated from the Email Studio work into their own PR against `main`, verified standalone: typecheck 19/19, unit 420/420, integration 75/75 (disposable Postgres + Redis), build 6/6. They are: the null product description/image editor crash; approved-version parsing that **fails the job rather than falling back to the live template**; unsupported merge tags no longer rendering as literal text; and the visible footer unsubscribe URL resolving instead of shipping as `{{unsubscribe_url}}`. The lifecycle test was verified non-vacuous by reverting the enrichment fix. **PR #36 wording corrected to a distinction that must be preserved:** product-safe is the real Shopify product image composited over generated scenery and is **not** a photograph of the product being naturally used in that setting; creative concept is illustrative generated campaign art and, because the current provider path does not receive the actual product image as a reference, **must not claim to accurately depict the merchant's product**. | **Neither PR is merged or deployed.** Reference-grounded generation is recorded as a **deferred `VisualProvider` capability, not an impossibility**: a provider/API that accepts the real product image as an input/reference is a later addition, and product fidelity must be proven before any claim such as "a person using this exact product" is allowed. The approved-version fail-closed behaviour is retained by instruction; the live-template fallback is not restored. The global CSS token inconsistency (`--sidebar`/`--background`/`--success` stored as HSL components) is corrected only inside the email Studio in #36 and remains an open design-system decision |
| 2026-09-22T12:32:23Z | _(branch `email-studio-block-ai`, draft PR #36)_ | **implemented, pending review** | Email Studio pass 2. **Two live defects found while building and fixed.** (1) The renderer resolved an unknown merge tag to the tag itself, so a token the sender does not populate — `{{firstname}}`, `{{city}}` — reached a customer's inbox as visible text; a token now resolves to value, then written fallback (`{{first_name|friend}}`), then the field's default, then nothing, never to itself, and the catalogue is limited to keys `send.worker` actually populates with a test asserting it. (2) **The footer's visible unsubscribe link shipped as `href="{{unsubscribe_url}}"`** — renderer-controlled, so it never passed through interpolation; the `List-Unsubscribe` header was always correct so mail-client unsubscribe worked, but the link in the body went nowhere. The older MJML path had it right, so this was a React-path regression; now resolved from the same variables, `#` in preview. **Shopify data tab added:** products are picked from the store rather than typed, a bound product's facts read as facts with their source, grids pick products, and the raw "Product IDs (one per line)" textarea is gone — those fields were futile anyway, since `resolveProduct` prefers store data and silently overwrote them. **Visual generation reshaped:** a request is a list of named slots each generated separately, so four labelled selectable assets rather than one collage; offer text in a prompt is refused before a paid call with the reason; `product_safe` composites the real product image and forbids the generator from drawing the product, `creative_concept` is labelled and may never be shown as product photography. Preflight gained two personalization checks. typecheck 19/19, unit 498/498, integration 81/81, build green. | **Draft PR only — not merged, not deployed.** Collection binding was deliberately NOT built: nothing resolves a collection at render time, so offering it would have been a promise the renderer cannot keep; the collections endpoint written for it was removed rather than left as dead code. Still absent and not claimed: merchant-visible frozen-vs-live state, contrast checking, per-campaign spend limit (the per-workspace daily image budget applies and a request is capped at four slots), drag-to-reorder. **No screenshots — this repository has no browser in its test setup**, so the visual walkthrough remains manual verification. Asset OCR, malware scanning, EXIF stripping and moderation remain absent as already recorded; embedded image text is prevented at the prompt rather than detected afterwards |
| 2026-09-22T11:42:13Z | _(branch `email-studio-block-ai`, draft PR)_ | **implemented, pending review** | Email Studio pass 1. **The broken editor is fixed and the cause was not what was assumed:** `blocks[3].props.description` was not legacy data but a live bug — `Product.description` and `Product.imageUrl` are nullable columns and `templates.getById` copied them onto product blocks on every fetch, so any product without a description broke its email, disabling preview and refusing save. Enrichment now omits absent values, and a schema-derived normalizer converts `null` to absent at exactly the keys already declared `.optional()`/`.default()`. **Ask-Joon scope became structural:** `containToScope` drops out-of-scope changes before they are applied and `scopeViolation` refuses the proposal if anything outside the scope moved anyway, so a block-scoped request cannot reach another block, the subject, or the block order; whole-email scope is never inferred from a selection. **The model can no longer write store facts:** product identity, title, price, description, image, grid contents, CTA destinations and image sources are stripped from model changes, and an added product block arrives with no product chosen. Audit at `docs/EmailStudioAudit-2026-09-22.md`. typecheck 19/19, unit 433/433, build green, 3/3 Studio integration on disposable Postgres; the lifecycle test was verified non-vacuous by reverting the fix. | **Draft PR only — not merged, not deployed.** One delivery behaviour change needs review: an approved email version that cannot be read back now fails the send job, where it previously fell through to the live template and would have delivered unapproved content. Not built and not claimed: `VisualProvider` abstraction, four-labelled-asset generation, product-safe vs creative-concept as a merchant choice, the three-tab Edit/Shopify data/Ask Joon inspector, friendly Shopify data tokens, merchant-visible frozen-vs-live state, and preflight expansion. **No screenshots were produced — this repository has no browser in its test setup**, so the visual walkthrough the brief asks for remains manual verification. Asset OCR, malware scanning, EXIF stripping and moderation remain absent, as already recorded above; no documentation correction was needed because the register was already honest about them |
