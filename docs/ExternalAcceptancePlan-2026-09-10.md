...
- [ ] Review one shadow invoice with every tester: campaign and journey attributed revenue, excluded cancelled orders, 5% displayed fee, 5/6/8% shadow variants and cap status. Verify deliberately-left-alone and control customers never enter attribution. Record comprehension and objections; never create a Billing API charge.
...
| Audience/billing model | Local automated and browser checks, 2026-09-14 | Local | `47a0079` through `6c923d6` | CODE COMPLETE; EXTERNAL VALIDATION PENDING | 258 tests passed; recursive typecheck and lint completed with zero errors; 58-page web build passed. Verify additive migrations in deployed environments, run real Shopify attribution through campaigns and journeys, validate pooled control evidence, and review shadow invoices with design partners before enabling billing. |
