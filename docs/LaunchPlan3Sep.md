...
- Subject experiments with explicit merchant application of a winner. Brand
  tone is consistently applied, but autonomous tone learning is post-launch.
...
- Autopilot, unapproved proactive outreach, or autonomous delivery. Joon may
  prepare brand-aware drafts in the approval queue; it cannot approve or send them.
...
The repository-only launch work is complete as of **5 September 2026**. The
...
App Bridge authentication is complete through tenant/role mapping and
Shopify-origin installation. The repository now also provides the intended
product transition: a compact embedded surface creates a five-minute,
single-use, digest-at-rest handoff; the full-screen surface requires Clerk and
atomically binds that account to the verified Shopify staff identity. Stage 1
remains open only for the real Shopify/Clerk/browser acceptance matrix.
...
| 1I — Full-screen account handoff      | Compact embedded entry creates a five-minute, single-use token whose digest is stored; Clerk redemption binds the verified Shopify staff identity and opens the full workspace                                                                      | Stage 1           | Done (`c8111ff`); live cross-browser validation remains in 1F                                                                                                                                                                                              |
| 1J — Unified onboarding readiness     | Final onboarding surface reports account, sync, consent, brand, Web Pixel, smart segments, sender DNS, suppression and delivery from one backend contract                                                                                           | Stages 1, 3 and 4 | Done (`c8111ff`); external DNS/pixel observations remain Stage 5 acceptance                                                                                                                                                                                |
| 1K — Migration assistance             | Merchants can request a consent-safe guided import of profiles, segments, templates and selected flows; requests are durable and idempotent while active                                                                                            | Stages 1 and 2    | Durable intake done (`c8111ff`); actual migrations remain concierge operations                                                                                                                                                                             |
...
| 3B — Dry-run report                   | Campaign and automation pages show requested/currently-sendable, mutually exclusive exclusions, estimated treatment/control and sender/domain state with zero provider calls                                                                        | Stage 3           | Done (`3d6f431`, `62109b8`): approval freezes sorted eligible IDs; finite campaigns receive an exact randomized 15% control quota; dispatch intersects with fresh safety eligibility                                                                       |
...
| 4A — Sender-domain onboarding         | Provider DNS records/status are created, displayed and refreshed inside onboarding and Integrations; campaign and automation workers require a verified From domain before live delivery                                                            | Stage 4           | Done (`c8111ff`); production provider/DNS validation remains                                                                                                                                                                                              |
...
| 6D — First-run smart segments         | Shopify taxonomy + catalog signals normalize into 20 verticals; verified purchase affinities and behavior segments refresh immediately after RFM; nightly work prepares additional evidence-backed segments and drafts for approval                  | Stage 6           | Done (`1497757`, `c26d2ec`); live-catalog validation remains in Stage 5                                                                                                                                                                                   |
| 6E — Brand voice on every new journey | Agent-created fixed skeletons contain timing only and immediately queue the same brand-aware email generator used by onboarding automations; v1 nodes are email-only; agent activation uses the same immutable approval checksum as the UI           | Stage 6           | Done (`c8111ff`); live provider generation remains Stage 5 acceptance                                                                                                                                                                                      |
...
2. Verify production health, configuration and worker boot with the v1 gate active.
3. Complete the real Shopify/Clerk/incognito/staff-account acceptance matrix.
4. Verify one merchant sender domain through DNS and Resend.
5. Confirm the deployed Web Pixel receives product, collection, page, search, cart and checkout events.
6. Run allowlist seed sends, provider failure drills and the six live journey fixtures.
7. Complete Level 2, legal/operator approval and listing assets, then submit.

Founder/external-console actions:
