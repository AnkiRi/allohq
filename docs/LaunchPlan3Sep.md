...
| 5A — Failure harness | Production drills cover 429/5xx backoff, accepted-then-timeout idempotency, permanent failures, post-fan-out approval mutation and unsubscribe/complaint after queueing; suppression and opt-out outrank legacy consent at the final delivery check | Stage 5 | Provider + approval + consent drills done; campaign/automation provider failures now enter BullMQ failed/DLQ state (transient retries, permanent stops). Live queue/database/webhook restart drills remain |
