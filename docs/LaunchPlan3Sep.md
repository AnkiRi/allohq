...
| 3B — Dry-run report | Campaign and automation pages show requested/currently-sendable, mutually exclusive exclusions, estimated treatment/control and sender/domain state with zero provider calls | Stage 3 | Core API + merchant UI done (`3a2025d` + current slice); frozen snapshot and offer/cost state remain |
...
| 5A — Failure harness | Production drills cover 429/5xx backoff, accepted-then-timeout idempotency, permanent failures, post-fan-out approval mutation and unsubscribe/complaint after queueing; suppression and opt-out outrank legacy consent at the final delivery check | Stage 5 | Provider + approval + consent drills done; queue/database/webhook restart drills remain |
...
| 6C — Claim audit | Public landing now states email-only, free v1, explicit approval and illustrative—not merchant—figures; multichannel, performance-billing and tone-learning promises removed | Stage 6 | Public surface done in current slice; authenticated dashboard copy audit remains |
