...
Joon is an operating surface for consequential retention work. It should feel watchful, exact and calm: a capable team has prepared the work, exposed the evidence and left the merchant in command. The landing page is the visual authority, while the authenticated application lowers the volume for sustained use. Pale paper, dense ink and restrained emerald establish the brand without competing with the task.
...
- Semantic color with stable meaning: amber/action, blue/measurement, green/outcome and red/failure.
- Sans-serif language for operation; monospaced type only for data, measurements and receipts.
- Two deliberate themes: paper and emerald in light mode; near-black and amber in dark mode.
...
- **Joon Emerald:** The light-theme brand accent, active-navigation state, progress cue and affirmative primary treatment. It signals Joon's presence or a healthy system state, not a request for approval.

### Secondary

- **Action Amber:** Work that needs the merchant now: approval, intervention, a pending decision or an attention badge. In dark mode, amber also becomes the brand accent.
...
- **Outcome Green:** Verified healthy outcomes and positive result states. It is semantically narrower than the brand emerald.
- **Failure Red:** Delivery failure, destructive action, invalid state or material risk. Do not use it for routine emphasis.

### Neutral

- **Paper:** The light application canvas and default reading field.
- **Surface:** Cards, popovers, inputs and other contained work areas on paper.
...
- **Navigation Black-Green:** The light-theme sidebar, creating a stable edge around the paper workspace.
...
**The Stable Meaning Rule.** Amber asks for action, blue measures, green reports an outcome and red reports failure. Never swap these roles to create visual variety.

**The Quiet Emerald Rule.** Emerald establishes identity and healthy continuity. Use it sparingly enough that an amber request still reads as the page's immediate action.

**The Theme Pair Rule.** Light mode is paper, ink and emerald; dark mode is near-black, pale ink and amber. Dark mode is a designed counterpart, not an inverted light palette.
...
Navigation reflects merchant tasks rather than the internal product model. Focus contains the immediate day, decisions and activity; Engage contains customer-facing work; Learn contains results and analysis; Create contains reusable inputs. Setup, integrations and settings remain utilities. On mobile, navigation becomes an off-canvas drawer with a dismissible scrim.
...
- **Ambient card** (`0 1px 2px rgba(12, 18, 14, 0.04), 0 10px 28px -24px rgba(12, 18, 14, 0.32)`): Resting light-theme cards and contained work surfaces.
- **Ambient card hover** (`0 1px 2px rgba(12, 18, 14, 0.05), 0 14px 34px -24px rgba(12, 18, 14, 0.38)`): Interactive light-theme cards, paired with a stronger border and no translation.
...
- **Primary:** Emerald in light mode and amber in dark mode for the leading non-decision action.
...
The sidebar is a dark, stable rail with 13px task labels, 17px line icons and uppercase 10px group annotations. Active items use the theme accent as a filled field and set `aria-current="page"`. Collapsed navigation preserves labels through hover and keyboard-focus tooltips. The top bar names the current task and gives it one plain-language description; global search and workspace health remain secondary.
...
- **Don't** use amber as generic decoration, blue as success, green as pending action or red as routine emphasis.
