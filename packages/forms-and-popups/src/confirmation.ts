...
export async function createConsentConfirmation(customerId: string, storeId: string, channel: "email" | "sms") {
  const token = randomBytes(32).toString("base64url");
  await prisma.$transaction([
    prisma.consentConfirmation.deleteMany({ where: { customerId, channel, usedAt: null } }),
    prisma.consentConfirmation.create({ data: { customerId, storeId, channel, tokenHash: digest(token), expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000) } }),
