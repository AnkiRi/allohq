...
export function TemplatePicker({ channel, currentTemplateId, currentTemplateName, onPick }: TemplatePickerProps) {
  ...
  return (
    <div className="space-y-2">
      <label className="block text-[10px] font-mono font-semibold text-gray-500 uppercase">
        Template
      </label>
      ...
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 bg-white text-sm font-mono text-left hover:border-gray-400 transition-all"
      >
        <Icon className={`w-3.5 h-3.5 ${meta.color} flex-shrink-0`} />
        <span className={`flex-1 truncate ${currentTemplateId ? "text-gray-900" : "text-gray-400"}`}>
          {displayName}
        </span>
        {currentTemplateId && (
      ...
      {open && (
        <>
          ...
          <div className="relative z-50 mt-1 border border-gray-200 rounded-xl bg-white shadow-xl overflow-hidden">
            {/* Search */}
            <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100">
              ...
              <input
                ...
                className="flex-1 text-sm font-mono text-gray-900 bg-transparent outline-none placeholder:text-gray-300"
                autoFocus
              />
              {search && (
              ...
              {filtered.length > 0 ? (
                filtered.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-3.5 h-3.5 ${isSelected ? meta.color : "text-gray-300"} flex-shrink-0`} />
                      <div className="flex-1 min-w-0">
                        <p className={`text-xs font-mono truncate ${isSelected ? "font-bold text-gray-900" : "text-gray-700"}`}>
                          {t.name}
                        </p>
                        {t.subject && (
                          <p className="text-[10px] font-mono text-gray-400 truncate">{t.subject}</p>
                        )}
                      </div>
                ...
                <div className="px-3 py-6 text-center">
                  <p className="text-xs font-mono text-gray-400">{meta.emptyText}</p>
