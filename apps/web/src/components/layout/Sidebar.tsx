...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, CircleGauge, FileText, Inbox, ListChecks, LogOut, Mail, MessageSquare, Settings, Sparkles, Users, Workflow } from "lucide-react";
...
export function Sidebar() {
  ...
  const navItem = (item: NavItem) => {
    ...
    return <Link key={item.name} href={item.href} onClick={close} aria-current={active ? "page" : undefined} className={cn("group/nav relative flex min-h-10 items-center rounded-[9px] text-[14px] transition-colors", collapsed ? "justify-center px-2" : "gap-3 px-3", active ? "bg-[var(--surface)] font-medium text-[var(--ink)]" : "text-white/78 hover:bg-white/[0.07] hover:text-white")}>
      ...
      {item.badge && pendingCount > 0 && <span className={cn("flex min-w-5 items-center justify-center rounded-full bg-[var(--attention-strong)] px-1.5 py-0.5 text-[11px] font-semibold text-[var(--ink)]", collapsed && "absolute -right-1 -top-1")}>{pendingCount > 99 ? "99+" : pendingCount}</span>}
  ...
  return <>
    ...
    <aside className={cn("app-sidebar fixed inset-y-0 left-0 z-50 flex flex-col border-r transition-[width,transform] duration-200 md:static md:translate-x-0", collapsed ? "w-[68px]" : "w-[204px]", isOpen ? "translate-x-0" : "-translate-x-full")}>
      <div className={cn("flex h-[68px] items-center", collapsed ? "justify-center px-3" : "px-6")}><Link href="/dashboard" className="flex items-center gap-2" aria-label="Joon home"><span className="h-2.5 w-2.5 rounded-full bg-[var(--attention-strong)]" />{!collapsed && <span className="text-[17px] font-medium tracking-[-0.02em] text-white">joon</span>}</Link></div>
      ...
      <nav className="app-sidebar-scroll flex-1 overflow-y-auto px-3" aria-label="Main navigation">
        ...
        {!collapsed && <details className="group rounded-[9px] text-white/78">
          ...
          <div className="ml-3 border-l border-white/12 pl-2">
            {moreItems.map(({ name, href }) => <Link key={href} href={href} onClick={close} className="block rounded-md px-3 py-1.5 text-[12px] hover:bg-white/[0.07] hover:text-white">{name}</Link>)}
          </div>
        </details>}
