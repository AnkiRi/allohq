...
const TONE_DIMENSIONS = [
  {
    ...
    left: {
      value: "formal",
      label: "Formal",
      example: "We are pleased to present our newest collection",
    },
    right: {
      value: "casual",
      label: "Casual",
      example: "You're going to love what we've been working on",
    },
  ...
  },
  {
    ...
    left: {
      value: "calm",
      label: "Calm",
      example: "Take your time exploring our thoughtfully curated range",
    },
    right: {
      value: "intense",
      label: "High Energy",
      example: "Don't miss out! Limited stock, grab yours now!",
    },
  ...
  },
  {
    ...
    right: {
      value: "intimate",
      label: "Warm",
      example: "We think you'll absolutely love this, picked just for you",
    },
  ...
  },
  {
    ...
    left: {
      value: "none",
      label: "Serious",
      example: "Our premium collection is now available for purchase",
    },
    right: {
      value: "heavy",
      label: "Playful",
      example: "Your cart misses you (seriously, it told us)",
    },
...
export function OnboardingWizard({
  ...
  const saveBrandReview = trpc.onboarding.saveBrandReview.useMutation({
    onSuccess: () => refetchStatus(),
  });
  const saveAutonomySetup = trpc.onboarding.saveAutonomySetup.useMutation({
    onSuccess: () => refetchStatus(),
  });
  const saveGuardrails = trpc.onboarding.saveGuardrails.useMutation({
    onSuccess: () => refetchStatus(),
  });
  const acknowledgeReport = trpc.onboarding.acknowledgeReport.useMutation({
    onSuccess: () => refetchStatus(),
  });
      ...
      <AnimatePresence mode="wait">
        {currentStep === 1 && (
          <StepWrapper key="step1">
            <BackgroundAnalysisStep
              status={status}
              storeId={storeId}
              onRefetch={() => refetchStatus()}
              onContinue={() => advance.mutate({ storeId, step: 2 })}
        ...
        {currentStep === 5 && (
          <StepWrapper key="step5">
            <GuardrailsStep
              ...
              onBack={() => goBack.mutate({ storeId, step: 4 })}
            />
          </StepWrapper>
        )}
        {currentStep === 6 && (
          <StepWrapper key="step6">
            <StoreReportStep
              storeId={storeId}
              onAcknowledge={() => acknowledgeReport.mutate({ storeId })}
              isAdvancing={acknowledgeReport.isPending}
              onBack={() => goBack.mutate({ storeId, step: 5 })}
            />
          </StepWrapper>
        )}
        {currentStep === 7 && (
          <StepWrapper key="step7">
            <FirstActionsStep
              ...
              onBack={() => goBack.mutate({ storeId, step: 6 })}
            />
          </StepWrapper>
        )}
      </AnimatePresence>
...
function BackgroundAnalysisStep({
  ...
  const analysisRows: {
  ...
  }[] = [
    {
      ...
      done: status?.brandVoiceComplete ?? false,
    },
    {
      key: "brandVisual",
      icon: Palette,
      label: "Extracting visual identity",
      done: status?.brandVisualComplete ?? false,
    },
    {
      key: "productImages",
      icon: ImageIcon,
      label: "Processing product images",
      done: status?.productImagesComplete ?? false,
    },
    {
      key: "rfm",
      icon: BarChart3,
      label: "Scoring customer health (RFM)",
      done: status?.rfmComplete ?? false,
    },
    {
      key: "baseline",
      icon: Boxes,
      label: "Capturing baseline metrics",
  ...
  const handleRetry = (key: RetryKey) => {
    ...
    const keys: RetryKey[] =
      key === "sync"
        ? ["sync", "brandVoice", "brandVisual", "productImages", "rfm", "baseline"]
        : [key];
    const t = Date.now();
    setRetriedAt((prev) => {
      const next = { ...prev };
  ...
  if (syncStuck)
    stuckSteps.push({ key: "sync", label: "Importing your store data", blocking: true });
  analysisRows.forEach((r) => {
    if (isStuck(r.key, r.done, ANALYSIS_STUCK_MS))
      stuckSteps.push({ key: r.key, label: r.label, blocking: false });
  });
...
function BrandReviewStep({
  ...
  } = (trpc as any).onboarding.getBrandReviewData.useQuery({ storeId }, { enabled: !!storeId }) as {
    data: any;
    isLoading: boolean;
    refetch: () => void;
  };
  ...
  const toneSummary = (() => {
    const fMap: Record<string, string> = {
      ...
      "semi-formal": "professional yet approachable",
      casual: "friendly and conversational",
      "very-casual": "relaxed and informal",
    };
    const wMap: Record<string, string> = {
      professional: "keeps a professional distance",
      friendly: "feels approachable",
      warm: "creates a sense of personal connection",
      intimate: "speaks directly to each customer like a friend",
    };
    const eMap: Record<string, string> = {
      calm: "with a calm, measured pace",
      moderate: "with balanced energy",
      high: "with enthusiasm and energy",
      intense: "with urgency and excitement",
    };
        ...
        {brandDocument.trim() && (
          <div className="flex items-center gap-3 mt-3">
            ...
            >
              {isAnalyzing ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" /> Reading your brand voice...
                </>
              ) : (
        ...
        {bp?.vocabulary && (
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1">
              {[
                ...((bp.vocabulary as any).preferredWords ?? []),
                ...((bp.vocabulary as any).brandTerms ?? []),
              ]
                .slice(0, 15)
          ...
          <div className="glass-card-static rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <Stat
                ...
                value={`${(report.revenueInsights.repeatPurchaseRate * 100).toFixed(1)}%`}
              />
            </div>
          </div>
...
function FirstActionsStep({
  ...
  const sender = senderDomains.get.useQuery({ storeId });
  const [domain, setDomain] = useState("");
  const invalidate = async () => {
    await Promise.all([readiness.refetch(), sender.refetch()]);
    await (utils as any).onboarding.readiness.invalidate({ storeId });
  };
  const configureDomain = senderDomains.configure.useMutation({ onSuccess: invalidate });
  const verifyDomain = senderDomains.verify.useMutation({ onSuccess: invalidate });
  const refreshDomain = senderDomains.refresh.useMutation({ onSuccess: invalidate });
  const requestMigration = (trpc.onboarding.requestMigrationAssistance as any).useMutation({
    onSuccess: invalidate,
  });
  const [migrationItems, setMigrationItems] = useState<string[]>([
    "profiles_and_consent",
    "segments",
    "templates",
    "selected_flows",
  ]);
  const data = readiness.data as any;
  const checks = data?.checks as
    | Record<string, { ready: boolean; detail: string; observed?: boolean }>
    | undefined;
  ...
  const rows: Array<[string, { ready: boolean; detail: string; observed?: boolean }]> = checks
    ? [
        ...
        ["Store data", checks.sync!],
        ["Email consent", checks.consent!],
  ...
  const migrationChoices = [
    ["profiles_and_consent", "Profiles & consent"],
    ["segments", "Segments"],
    ["templates", "Templates"],
        ...
        <section className="rounded-xl bg-[#FAF9F7] p-5">
          ...
          {data.migrationRequest ? (
            ...
            <>
              <div className="mt-3 flex flex-wrap gap-2">
                {migrationChoices.map(([value, label]) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={migrationItems.includes(value)}
                      onChange={() => toggleMigrationItem(value)}
                    />{" "}
                    {label}
                  </label>
                ))}
              ...
              <button
                disabled={!migrationItems.length || requestMigration.isPending}
                onClick={() =>
                  requestMigration.mutate({
                    storeId,
                    sourcePlatform: data.currentEmailPlatform,
