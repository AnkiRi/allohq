            ...
            <div className="rounded-lg border border-border bg-muted/40 p-3 text-[11px] leading-relaxed text-muted-foreground">
              ...
              <div className="mt-4 border-t border-border pt-3" aria-label="Domain warm-up ramp">
                ...
                <div className="mt-4 rounded-lg border border-border bg-[var(--surface)] p-3">
                  <div className="flex items-start justify-between gap-4">
                    <div><span className="font-medium text-foreground">Settled sending day {new Date(data.reputation.windowStartsAt).toISOString().slice(0, 10)} · {data.reputation.provider}</span><p className="mt-1 text-[10px]">{data.reputation.attempted.toLocaleString("en-IN")} attempted · {(data.reputation.bounceRate * 100).toFixed(2)}% bounced · {(data.reputation.complaintRate * 100).toFixed(3)}% complained</p></div>
