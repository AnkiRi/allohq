        ...
        <div className="mt-4 space-y-3">
          ...
          {data.provider === "ses" && (
            <div className="rounded-lg border border-border bg-muted/40 p-3 text-[11px] leading-relaxed text-muted-foreground">
              ...
              <div className="mt-4 border-t border-border pt-3" aria-label="Domain warm-up ramp"><div className="flex items-center justify-between"><span className="font-medium text-foreground">Safe volume ramp</span><span>{data.warmup?.pausedAt ? "Paused" : data.warmup?.heldUntil ? "Held" : data.warmup ? `Day ${data.warmup.healthyDay}` : "Starts after verification"}</span></div><div className="mt-3 grid grid-cols-4 gap-2">{[500, 1000, 2000, 4000].map((cap, index) => { const reached = (data.warmup?.healthyDay ?? 0) > index; const current = (data.warmup?.healthyDay ?? 0) === index + 1; return <div key={cap} className={`rounded-lg border px-2 py-2 text-center ${current ? "border-[var(--attention)] bg-[var(--attention-soft)]" : reached ? "border-[var(--success)]/30 bg-[var(--success-soft)]" : "border-border bg-[var(--surface)]"}`}><span className="block font-mono text-[12px] text-foreground">{cap.toLocaleString("en-IN")}</span><span className="mt-0.5 block text-[10px]">day {index + 1}</span></div>; })}</div><p className="mt-3 text-[10px]">Volume grows only after a reconciled healthy sending day. Holds, pauses and deferred recipients remain visible here.</p></div>
            </div>
          )}
