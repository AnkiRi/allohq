...
import { AlloAIPanelEmbedded } from "@/components/ai/AlloAIPanel";
...
// ---------------------------------------------------------------------------
// Dashboard Right Panel — data & insights (scrollable)
// ---------------------------------------------------------------------------
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants}>
        <h1
          className="text-[22px] font-bold tracking-[-0.03em] text-foreground"
      ...
      {/* 2. KPI CARDS — 2x2 grid to fit narrower width */}
      <motion.div variants={itemVariants} className="grid grid-cols-2 gap-3">
        {(() => {
          ...
          <Link
            key={i}
            href={kpi.href}
            className="group relative rounded-2xl p-4 border border-black/5 transition-all duration-300 hover:-translate-y-0.5 cursor-pointer"
            style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(20px)" }}
          >
            <div className="flex items-center justify-between mb-2">
              <span
                className="text-[9px] font-mono uppercase tracking-[0.1em] text-gray-400"
                style={{ fontFamily: "'JetBrains Mono', monospace" }}
              >
                {kpi.label}
              </span>
            ...
            <div className="flex items-end gap-2">
              <AnimatedCounter
                value={kpi.value}
                prefix={kpi.prefix || ""}
                className="text-[22px] font-bold tracking-[-0.03em] tabular-nums text-foreground"
                duration={1.2}
              />
              <span
                className={`inline-flex items-center gap-0.5 text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded-full mb-0.5 ${
                  kpi.neutral
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span
              className="text-[10px] font-mono uppercase tracking-[0.12em] font-bold text-gray-400"
              style={{ fontFamily: "'JetBrains Mono', monospace" }}
            >
              Daily Briefing
            </span>
            <span className="text-[9px] font-mono text-gray-300">{briefingDate}</span>
          </div>
        </div>
        ...
        <div className="space-y-0">
          {attentionItems.map((item, i) => {
            ...
            return (
              <div
                key={`priority-${i}`}
                className={`flex items-start gap-3 py-3 ${i < attentionItems.length - 1 || (narrative && narrative.length > 1) ? "border-b border-black/[0.04]" : ""}`}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[12px] font-semibold text-foreground">{item.text}</div>
                  <p className="text-[10px] text-gray-500 mt-0.5">{item.detail}</p>
                </div>
                <Link
                  href={item.href}
                  className="text-[9px] font-mono font-semibold px-2.5 py-1 rounded-lg bg-[#2c2418] text-[#faf8f5] hover:bg-[#2c2418]/90 transition-colors flex-shrink-0 mt-1"
                >
                  {item.action}
                </Link>
              </div>
          ...
          {narrative && narrative.length > 1 && narrative.slice(1).map((para, i) => {
            ...
            return (
              <div
                key={`narrative-${i}`}
                className={`flex items-start gap-3 py-3 ${i < (narrative.length - 2) ? "border-b border-black/[0.04]" : ""}`}
              >
                <div
                  className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                ...
                <p className="text-[11px] text-foreground/80 leading-relaxed flex-1">{para}</p>
              </div>
            );
          })}
          ...
          {attentionItems.length === 0 && (!narrative || narrative.length <= 1) && (
            <div className="flex items-start gap-3 py-3">
              <div className="w-6 h-6 rounded-full bg-[#B8963E]/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                <Info className="w-3 h-3 text-[#B8963E]" />
              </div>
        ...
        >
          ...
          <div className="space-y-0">
            {[
              ...
              <div
                key={i}
                className={`flex items-start gap-3 py-2.5 ${i < arr.length - 1 ? "border-b border-black/[0.04]" : ""}`}
              >
                <CheckCircle className="w-3.5 h-3.5 text-[#6B7A2F] mt-0.5 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-[12px] font-medium text-foreground">{item.action}</div>
          ...
          {tokenUsage && tokenUsage.totalCalls > 0 && (
            <div className="mt-3 pt-2.5 border-t border-black/[0.06] flex items-center gap-3">
      ...
      >
        ...
        {programs && programs.length > 0 ? (
          <div className="space-y-0">
            {programs.map((p, i) => (
              <Link
                key={p.id}
                href="/automations"
                className={`flex items-center justify-between py-2 hover:bg-white/30 -mx-2 px-2 rounded-lg transition-colors ${i < programs.length - 1 ? "border-b border-black/[0.03]" : ""}`}
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <span className="text-[11px] font-mono text-foreground truncate">{p.name}</span>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="text-[9px] font-mono text-muted-foreground capitalize">{p.status}</span>
                  {p.status === "ready" && (
          ...
          <div className="py-4 text-center">
            <p className="text-[11px] text-muted-foreground font-sans">Allo is setting up automations...</p>
          </div>
        )}
      </motion.div>
      ...
      >
        ...
        {segmentDist && segmentDist.length > 0 ? (
          <div className="space-y-2.5">
              ...
              return (
                <div key={s.segment} className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono text-muted-foreground">{s.customerCount}</span>
                      <span className="text-[9px] font-mono text-muted-foreground/60">{Math.round(pct)}%</span>
                    </div>
                  </div>
            ...
            {customerStats && (
              <div className="pt-2.5 mt-1 border-t border-black/[0.04] flex items-center gap-4 text-[9px] font-mono text-muted-foreground">
          ...
          <p className="text-[11px] text-muted-foreground font-sans py-4">Run RFM analysis to see segments.</p>
        )}
      </motion.div>
    </motion.div>
  );
...
// ---------------------------------------------------------------------------
// Dashboard-D: Agent-first layout — AI Panel center, data right
// ---------------------------------------------------------------------------

export default function DashboardDPage() {
  return (
    <div className="flex h-full -m-3 sm:-m-4 md:-m-6">
      {/* AI Panel — center, takes ~55% */}
      <div className="flex-[55] min-w-0 border-r border-border">
        <AlloAIPanelEmbedded />
      </div>

      {/* Dashboard data — right, takes ~45% */}
      <div className="flex-[45] min-w-0 overflow-y-auto p-4 md:p-5">
        <DashboardPanel />
      </div>
    </div>
  );
