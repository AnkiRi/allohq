...
export default function ActivityPage() {
  ...
  return <div className="space-y-6">
    ...
    <MetricStrip items={[{ label: "Events shown", value: items.length }, { label: "Waiting on you", value: needsYou.length }, { label: "Delivery events", value: sent.length }, { label: "Revenue recorded", value: money(revenue) }]} />
    <div className="app-tab-bed" role="tablist" aria-label="Activity views">{([ ["all", "All"], ["needs_you", `Needs you · ${needsYou.length}`], ["sent", "Delivery"], ["analysis", "Analysis"] ] as const).map(([id, label]) => <button key={id} role="tab" aria-selected={kind === id} className="app-tab" onClick={() => setKind(id)}>{label}</button>)}</div>
    <div className="grid min-h-[560px] gap-4 lg:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <aside className={`${inspectorOpen ? "fixed inset-x-3 bottom-3 top-20 z-50 overflow-auto" : "hidden"} lg:sticky lg:top-0 lg:z-auto lg:block lg:self-start`} aria-label="Activity receipt"><Surface className="overflow-hidden">{selected && selectedRow ? <>
        ...
        <dl className="divide-y divide-border text-[13px]">{[["Category", selectedRow.category ?? "—"], ["Autonomy", selectedRow.tier ? TIER_NOTE[selectedRow.tier] ?? selectedRow.tier : "—"], ["Result", selectedRow.actionTaken?.replace(/_/g, " ") ?? "Recorded"], ["Revenue", money(selected.rows.reduce((sum, row) => sum + (row.revenue ?? 0), 0))], ["Latest receipt", selectedRow.id]].map(([label, value]) => <div key={label} className="grid grid-cols-[100px_1fr] gap-3 px-5 py-3"><dt className="text-muted-foreground">{label}</dt><dd className={label === "Latest receipt" ? "break-all font-mono text-[11px]" : "font-medium"}>{value}</dd></div>)}</dl>
