...
function cohortTone(cohort: string) {
  const value = cohort.toLowerCase();
  if (/champion|loyal|potential/.test(value)) return "bg-[var(--success-soft)] text-[var(--success)]";
  if (/risk|hibernat/.test(value)) return "bg-[var(--attention-soft)] text-[var(--attention)]";
  if (/lost|bounce|complaint/.test(value)) return "bg-[var(--risk-soft)] text-[var(--risk)]";
  if (/new|subscriber/.test(value)) return "bg-[var(--evidence-soft)] text-[var(--evidence)]";
  return "bg-[var(--surface-soft)] text-muted-foreground";
}
